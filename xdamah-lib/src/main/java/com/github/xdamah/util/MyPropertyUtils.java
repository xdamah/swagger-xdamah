...
public class MyPropertyUtils {
	...
	public static Map<String, Type> buildPropertyTypes(Class beanClass) {
		...
		for (Method declaredMethod : declaredMethods) {
			...
			else if(declaredMethodName.startsWith("is")&& genericParameterTypes.length==0 && !genericReturnType.getTypeName().equals("void"))
			{
			...
			else if(declaredMethodName.startsWith("set")&& genericParameterTypes.length==1 && genericReturnType.getTypeName().equals("void"))
			{
				if(declaredMethodName.length()>3)
				{
					String temp=declaredMethodName.substring(3);
		...
		if(test)
		{
			...
			for (Entry<String, Type> entry : entrySet) {
				...
				if(type!=null)
				{
					...
					if(rawType==null)
					{
						throw new RuntimeException(propertyName+" in "+beanClass+" getRawType() logic needs improvement");
					}
				}
			}
		}
