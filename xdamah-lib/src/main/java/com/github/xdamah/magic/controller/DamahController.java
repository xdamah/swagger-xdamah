...
public class DamahController {
	...
	public ResponseEntity handleRequest(HttpServletRequest request, HttpServletResponse response) throws IOException, ClassNotFoundException, InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException, AssertionError, ServletException
	{
		...
		RequestBodyBuilder requestBodyBuilder= new RequestBodyBuilder(operation, modelPackageUtil,
				objectMapper, mappingJackson2XmlHttpMessageConverter, openApi, conversionService);
			...
			if(typename!=null  && paramWrapperBean!=null)
			{
				Map<String, Type> propertyTypesMap = MyPropertyUtils.buildPropertyTypes(paramwrapperClass);
					...
					//we are here not interested in whatWasSet
					 Object whatWasSet = new ParameterProcessorForWrapperBean(this.path, 
							 this, paramWrapperBean, this.conversionService,
							 propertyTypesMap).process(request,  operationParameter);
				}
			...
			if(methodAndIndexes.getParamIndex()!=-1)
			{
				...
				if(paramWrapperBean!=null)
				{
					if(isArraySingleParam!=null)
					{
						...
						{
							converted=singleParamConverter.convertToTypeDForSingleParam((List<String>)paramWrapperBean, 
									refOperationParameterIfThereIsASingleParameter, 
		...
		{
			responseEntity=new ResponseEntity<>(ret, headers, HttpStatus.valueOf(Integer.parseInt(statusCodeOf200Or201)));
		}
	...
	private String getResponseContentTypeToUse(ApiResponse apiResponseFor200Or201, Object ret, Set<String> acceptHeaders) throws IOException {
		String contentTypeTouse=null;
				...
				else
				{
					...
					else
					{
						...
						for (String responseContentType : contentKeySet) {
							...
							{
								if(responseRefType.equals(Constants.COMPONENTS_SCHEMA_PREFIX+ret.getClass().getSimpleName()))
								{
		...
		for (String responseContentType : contentKeySet) {
			...
			if(mediaType!=null)
			{
				...
				if(schema!=null)
				{
					...
					if(type!=null && type.equals("string") && format!=null && (format.equals("binary")||format.equals("byte")))
					{
						oks++;
					}
				}
	...
	private MethodAndIndexes getMethod(Class<?> requestBodyTargetType, Parameter refOperationParameterIfThereIsASingleParameter,
			Class paramClass) throws ClassNotFoundException, NoSuchMethodException {
...
Map<String, Object> extensions = operation.getExtensions();
		...
		if(extensions!=null)
		{
			...
			if(serviceInfo!=null)
			{
				...
				if(serviceInfo.endsWith(")"))
				{
					...
					String argTypesSection=serviceInfo.substring(openBraceIndex+1, serviceInfo.length()-1);//we dont want closing brace or openingbrace
	...
	private Method getActualMethod(String methodName, Class[] argTypes, Class serviceClass, int bodyArgIndex) throws NoSuchMethodException {
	...
	public void setMappingJackson2XmlHttpMessageConverter(MappingJackson2XmlHttpMessageConverter mappingJackson2XmlHttpMessageConverter) {
	...
	interface StreamReaderToTarget {
		...
		Object read(InputStreamReader isr, Class<?> targetType) throws IOException, StreamReadException, DatabindException;
