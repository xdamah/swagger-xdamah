	...
	public Object buildRequestBody(HttpServletRequest request,  String contentType)
		...
		if(contentType!=null)
		{
			...
			if(targetType!=null||(discriminatorPropertyName!=null && this.requestTypeInfo.getOneOfRefs()!=null))
			{
						...
						if(targetType==null)
						{
							...
							if(discriminatorNode!=null && discriminatorNode instanceof TextNode)
							{
								...
								for (String ref : this.requestTypeInfo.getOneOfRefs()) {
									if(ref.startsWith(com.github.xdamah.constants.Constants.COMPONENTS_SCHEMA_PREFIX)
											||
											ref.startsWith(com.github.xdamah.constants.Constants.COMPONENTS_SCHEMA_PREFIX1))
									{
										String simpleClassName = modelPackageUtil.simpleClassNameFromComponentSchemaRef(ref);
				...
				{
					reqBody = ifStringElse(request, targetType, mappingJackson2XmlHttpMessageConverter.getObjectMapper()::readValue);
				...
				else if(contentType.equals(org.springframework.http.MediaType.APPLICATION_FORM_URLENCODED_VALUE))
				{
					...
					FormProcessor formProcessor = new FormProcessor(request,this.modelPackageUtil, this.openApi, this.conversionService);
				...
				else if(contentType.startsWith(org.springframework.http.MediaType.MULTIPART_FORM_DATA_VALUE))
				{
					Map<String, Schema> schemas = this.openApi.getComponents().getSchemas();
					Schema schema = schemas.get(targetType.getSimpleName());
					MultiPartFormProcessor formProcessor = new MultiPartFormProcessor(request,this.modelPackageUtil, this.openApi, this.conversionService);
